...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	// Allocate matrices to gpu first
	Matrix M_dev = AllocateDeviceMatrix(M);
	Matrix N_dev = AllocateDeviceMatrix(N);
	Matrix P_dev = AllocateDeviceMatrix(P);

	// cuda memcpy. copy cpu data to gpu
	// (mdevice, mhost)
	CopyToDeviceMatrix(M_dev, M);
	CopyToDeviceMatrix(N_dev, N);

	
	// exec config + run
	// assumes square matrices
	dim3 dimBlock(MATRIX_SIZE, MATRIX_SIZE); // 16x16 threads
	dim3 dimGrid(1, 1);

	MatrixMulKernel<<< dimGrid, dimBlock >>>(M_dev, N_dev, P_dev);
	...
	// copy from gpu
	// (Mhost, Mdevice)
	CopyFromDeviceMatrix(P, P_dev);

	//de-alloc memory
	cudaFree(M_dev.elements);
	cudaFree(N_dev.elements);
	cudaFree(P_dev.elements);
}
