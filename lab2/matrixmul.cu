...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	// set tile width and height to 16 = 256 threads for each block
	dimBlock.x = tile_width;
	dimBlock.y = tile_width;
	dimBlock.z = 1;
	// calculate grid size. so it covers the whole out matrix
	dimGrid.x = (P.width + tile_width - 1) / tile_width;
	dimGrid.y = (P.height + tile_width - 1) / tile_width;
	dimGrid.z = 1;
